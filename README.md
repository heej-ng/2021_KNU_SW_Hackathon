...
### :mag: 서비스 기능
- 회원가입 · 로그인 · 로그아웃 기능
> 학생들은 회원가입을 할 때 이름, 비밀번호, 학번, 전공 정보를 기입해야 한다. 그리고 로그인과 로그아웃 기능이 제공된다.
- 강의실 입장 기능
> 각 강의실에 학생들이 입장하면 해당 강의실 정보가 띄워지고, 학생들은 on/off버튼을 눌러 간편하게 출석을 할 수 있다. 또한, 온라인 강의 버튼을   누르면 해당 강의실 수업의 zoom/webex 링크가 연동되어 학생들은 온라인 화상 강의를 들을 수 있다. 학생들은 강의를 듣다가 문의사항이 있으면 버  튼을 통해 바로 과사에 이동할 수 있거나 강의실을 나갈 수도 있다.
- 같은 강의실 내 유저 간 채팅 기능
> 동일한 강의실에 들어간 학생들 끼리 단체 채팅이 가능하여 학생들은 교수님, 친구들끼리 즉각적으로 소통이 가능하여 교수님께 모르는 사항을 질문할 수 있고, 학우들 간에 교우 관계를 다질 수도 있다.
- 과 사무실/교수 연구실 입장 기능
> 과 사무실/교수 연구실에 들어가면 정보(주소, 전화번호, 팩스번호)를 얻어서 학생들은 학교 생활에 필요한 정보를 얻을 수 있다. 
- 층수 이동 기능
> 엘레베이터를 이용하여 층수를 이동할 수 있다.
